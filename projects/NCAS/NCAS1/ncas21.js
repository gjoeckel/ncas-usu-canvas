...
(function () {
  ...
  */
  ...
  /**
   * Check if user was redirected from course home page
   * Uses referrer check and overlay flag timing for reliability
   */
  function isRedirectFromCourseHome(courseId) {
    if (!courseId) return false;
    
    // Method 1: Check referrer
    const referrer = document.referrer;
    const courseHomeUrl = `https://usucourses.instructure.com/courses/${courseId}`;
    const courseHomeUrlSlash = `https://usucourses.instructure.com/courses/${courseId}/`;
    const referrerMatches = referrer === courseHomeUrl || referrer === courseHomeUrlSlash;
    
    // Method 2: Check if overlay was just set (within last 2 seconds)
    // This catches redirects even if referrer is not available
    ...
    const recentlySet = overlayStartTime && (Date.now() - parseInt(overlayStartTime, 10)) < 2000;
    
    // Return true if either method indicates redirect from home
    return referrerMatches || (overlayActive && recentlySet);
  }
   ...
   *       - Redirected to Core Skills page (only if redirected from course home)
   *       - loadUsuCanvasUserCoreSkills() triggered
   *       - Overlay fades out after Core Skills setup completes
   * 
   * Note: Flagged users can navigate directly to Start Here (via nav bar, bookmark, etc.)
   * without being redirected. They are only redirected if coming from course home.
   */
  function onCourseLoad() {
    ...
    if (isOnStartHere) {
      // Check if user was redirected from course home
      const isRedirectFromHome = isRedirectFromCourseHome(courseId);
      const usuCanvasUserFlag = sessionStorage.getItem('usu-canvas-user') === 'true';
      
      // If user was redirected from home AND already has flag, redirect immediately
      // This handles the case where a flagged user refreshes or returns to course home
      if (isRedirectFromHome && usuCanvasUserFlag) {
        log('Flagged user redirected from course home - redirecting to Core Skills immediately');
      ...
      // Skip detection if user already has flag and navigated directly (not from redirect)
      if (!usuCanvasUserFlag || isRedirectFromHome) {
        setTimeout(async () => {
        ...
        // Flagged user navigated directly to Start Here (not from redirect)
        // Allow them to stay on Start Here page
        log('Flagged user navigated directly to Start Here - allowing access without redirect');
        ...
        if (overlayActive) {
          waitForPageBodyLoaded().then(() => {
            ...
            if (overlay) {
              ...
              sessionStorage.removeItem('ncademi-overlay-active');
              sessionStorage.removeItem('ncademi-overlay-start-time');
              log('Start Here page body loaded, overlay faded out (flagged user direct navigation)');
            }
          });
        }
      }
  ...
  function shouldRunQuizButtonUpdate() {
    ...
    */
    ...
    if (document.querySelector('a.quiz-button')) {
      return true;
    }
  ...
  async function updateSkillPageQuizButtons(options = {}) {
    ...
    skillQuizButtonUpdatePromise = (async () => {
      ...
      if (missingAssignmentIds.length > 0 && signedIn) {
