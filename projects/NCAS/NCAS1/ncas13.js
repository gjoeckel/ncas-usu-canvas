/**
 * NCADEMI Navigation JavaScript (v13)
...
(function () {
  ...
  const LOG_PREFIX = "🚀 NCADEMI Navigation v13";
  ...
  function applyStatusToCoreSkillsLinks(submissions, assignmentsMap, linkMap) {
    ...
    // Ensure all Core Skills links have status labels (default to Pending if no status)
    linkMap.forEach((link, assignmentId) => {
      if (!link.hasAttribute('data-status')) {
        // Link has no status yet, default to pending
        link.setAttribute('data-status', 'pending');
        const linkText = link.textContent.trim() || link.getAttribute('title') || 'Link';
        // Add aria-label with pending status - link text will be read, but we need to include status for accessibility
        if (!link.getAttribute('aria-label') || !link.getAttribute('aria-label').includes('Status:')) {
          link.setAttribute('aria-label', `${linkText}, Status: Pending`);
        }
      }
    });
  }
  ...
  function populateStatusColumn() {
    ...
    assignmentRows.forEach(row => {
      ...
      // Icon is decorative (nearby text provides alternative text) - CSS ::after pseudo-element
