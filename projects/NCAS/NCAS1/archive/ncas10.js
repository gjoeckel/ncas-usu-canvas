/**
 * NCADEMI Navigation JavaScript (v10)
...
(function () {
  ...
  const LOG_PREFIX = "🚀 NCADEMI Navigation v10";
  ...
  function setupCanvasNavigationListener() {
    ...
    if (!window.ncademiDOMReadyListenerSet) {
      document.addEventListener('DOMContentLoaded', function() {
        const courseId = getCourseId();
        if (courseId && !isAdmin() && window.ncademiInitializedV10) {
  ...
  if (window.ncademiInitializedV10) {
  ...
  waitForDOM(() => {
    ...
    window.ncademiInitializedV10 = true;
    logOK("Initialized: v10 nav + A11Y + Container Width Enforcement + Skip Link");
  });
})();