...
(function () {
  ...
  function getExistingHeader() {
    ...
    if (existingHeader.parentNode === document.body && existingHeader === document.body.firstChild) {
  ...
  function setupCanvasNavigationListener() {
        ...
        window.Backbone.history.navigate = function(fragment, options) {
          ...
          if (options && options.trigger) {
            setTimeout(() => {
              const courseId = getCourseId();
              if (courseId && !isAdmin()) {
                log("Canvas navigation detected, updating header");
                ensureHeaderNav();
              }
            }, 100); // Small delay to let Canvas update DOM
          }
    ...
    if (!window.ncademiDOMReadyListenerSet) {
      document.addEventListener('DOMContentLoaded', function() {
        const courseId = getCourseId();
        if (courseId && !isAdmin() && window.ncademiInitializedV8) {
