/**
 * NCADEMI Navigation JavaScript (NCAS20)
...
(function () {
  ...
  const LOG_PREFIX = "🚀 NCADEMI Navigation NCAS20";
  ...
  /**
   * Redirect flow:
   * 1. Course home ALWAYS redirects to Start Here page
   * 2. On Start Here page, check if user is logged in:
   *    - If not logged in: Do nothing (stay on Start Here)
   *    - If logged in: Redirect to Core Skills with aria message and flag
   */
  function handleInitialRedirects() {
    ...
    // Step 1: Redirect all users from course home to Start Here (ALWAYS)
    if (isOnCourseHome) {
      log(`Redirecting all users to Start Here from course home`);
      if (document.body) {
    ...
    // Step 2: On Start Here page, check if user is signed in and redirect to Core Skills
    if (isOnStartHere) {
      // Check if we've already processed this redirect (prevent redirect loops)
      const redirectProcessed = sessionStorage.getItem('ncademi-start-here-processed') === 'true';
      
      // Wait a moment for sign-in state to be determined, then check
      setTimeout(async () => {
        const signedIn = await waitForSignedInState({ timeout: 2000 });
        
        if (signedIn && !redirectProcessed) {
          // User is signed in - redirect to Core Skills with aria message
          log('User is signed in, redirecting from Start Here to Core Skills');
          
          // Create and inject aria live region for screen readers
          let ariaLive = document.getElementById('ncademi-redirect-aria');
          if (!ariaLive) {
            ariaLive = document.createElement('div');
            ariaLive.id = 'ncademi-redirect-aria';
            ariaLive.setAttribute('role', 'status');
            ariaLive.setAttribute('aria-live', 'polite');
            ariaLive.setAttribute('aria-atomic', 'true');
            ariaLive.className = 'screenreader-only';
          ...
          ariaLive.textContent = 'Redirecting to the Core Skills page.';
          
          // Set flag to indicate user is logged in (skip Canvas check on Core Skills page)
          sessionStorage.setItem('ncademi-logged-in-redirect', 'true');
          
          // Mark as processed to prevent loops
          sessionStorage.setItem('ncademi-start-here-processed', 'true');
          
          // Small delay to ensure aria message is announced
          setTimeout(() => {
            if (document.body) {
              document.body.style.display = 'none';
            }
          ...
          // User is not signed in - do nothing, stay on Start Here page
          log('User is not signed in, staying on Start Here page');
        }
      }, 100);
  ...
  async function injectSignInBanner() {
    ...
    const loggedInRedirect = sessionStorage.getItem('ncademi-logged-in-redirect') === 'true';
    if (loggedInRedirect) {
      return; // User was redirected from Start Here, already verified as logged in
    }
  ...
  // Handle redirects: course home always redirects to Start Here, then signed-in users redirect to Core Skills
